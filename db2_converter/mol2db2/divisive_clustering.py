...
import sys
import logging
from db2_converter.mol2db2 import pca  # for bisective PCA part of clustering
...
def divisiveClustering(
  ...
  while len(clusters) < numClusters:  # until we have enough clusters
    ...
    try:
      ...
      if verbose:
        logging.debug("convergence problem during clustering. quitting with ", end=' ')
        logging.debug(len(clusters), " clusters, which should be enough for anybody")
      break  # quit now if we can't converge. means cluster too similar.
    except SplitZeroError:
      if verbose:
        logging.debug("projection/split problem during clustering. quitting with ", end=' ')
        logging.debug(len(clusters), " clusters, which should be enough for anybody")
      break  # quit now if we can't converge. means cluster too similar.
    #logging.debug clusters #debugging
    if verbose:
      logging.debug("size of each cluster", end=' ')
      for cluster in clusters:
        logging.debug(len(cluster), end=' ')
