...
from db2_converter.amsol.mol2amsol import (
    read_Mol2_file,
    write_mol2,
)  ## this is a libary Trent Balius and Sudipto Mukherjee wrote.
...
def process_amsol_file(file, outputprefix, watorhex):
    ...
    logger.debug("**** starting the function process_amsol_file() ****")
    if not exist_size(file):
    ...
    for line in lines:
        ...
        #
        # infos originally coming from the AMSOL7.1 input, which are relogger.debuged in the AMSOL7.1 output-file:
        # i.e., molecule's name and number of atoms in this molecule:
        # i.e., ZINC00000000.0.1_1 37
        if (
            len(linesplit) == 2 and linesplit[1].isdigit() and name == ""
        ):  # get the name and atom cout.
            file2.write(line)
            name = linesplit[0]
        ...
        if (
            len(linesplit) == 9
            and linesplit[0].isdigit()
            and (linesplit[4] != "*")
            and not linesplit[1].isdigit()
        ):  ## this gets that per-atom break-down of solvation calculation
            file2.write(line)
    ...
    logger.debug(" **** The function process_amsol_file() was finished. ****")
...
def diff_amsol71_files(
    atom_listwat, totwat, atom_listhex, tothex, name, numatom, outputprefix
):

    logger.debug("**** starting the function diff_amsol71_files() ****")
    ...
    if len(atom_listwat) != len(atom_listhex):
        logger.error(
            "Error: len(atom_listwat) != len(atom_listhex):"
            + str(len(atom_listwat))
            + " != "
            + str(len(atom_listhex))
        )
        sys.exit()
    ...
    if N != numatom:
        ...
        logger.error(
            "Error: len(atom_listwat) != numatom: " + str(N) + " != " + str(numatom)
        )
        sys.exit()

    logger.debug("atom_listwat[i][j] + -- + atom_listhex[i][j] +;   ::::")
    for i in range(N):
        # amsol-mod4 for j in range(0,8):
        for j in range(0, 9):
            logger.debug(
                f"{atom_listwat[i][j]} -- {atom_listhex[i][j]};"
                )
    ...
    for i in range(N):
        Chghex.append(
            0.0
        )  # list of partial atomic charges from AMSOL7.1 SM5.42R solvation calculation in hexadecane solvent
        Polhex.append(
            0.0
        )  # list of atomic polar contributions to solvation free energy: G_P obtained for hexadecane solvent
        SAA.append(
            0.0
        )  # list of atomic surface area contributions taken from hexadecane output
        Sigmahex.append(0.0)  # list of sigma coefficients
        Apolhex.append(
            0.0
        )  # list of atomic apolar contributions to solvation free energy: SS G_CDS obtained for hexadecane solvent

        Polwat.append(
            0.0
        )  # list of atomic polar contributions to solvation free energy: G_P obtained for water solvent
        Apolwat.append(
            0.0
        )  # list of atomic apolar contributions to solvation free energy: SS G_CDS obtained for water solvent

        diff_Pol.append(
            0.0
        )  # list of differences of the atomic polar contributions to solvation free energy in water and hexadecane: wat - hex
        diff_Apol.append(
            0.0
        )  # list of differences of the atomic polar contributions to solvation free energy in water and hexadecane: wat - hex
        diff_AtomicSolv.append(
            0.0
        )  # list of differences of the atomic solvation free energies (polar + apolar contributions) in water and hexadecane: wat - hex
    ...
    for i in range(N):
        ...
        Chghex[i] = float(
            atom_listhex[i][2]
        )  # partial charges (CM2: Truhlar's charge model 2 charges) of each atom in hexadecane solvent
        Polhex[i] = float(
            atom_listhex[i][3]
        )  # atomic polar contributions to solvation free enthalpy in hexadecane: Polarization Free Energy (G_P) in kcal/mol
        SAA[i] = float(
            atom_listhex[i][4]
        )  # atomic surface area contibutions in Angstrom^2
        ...
        Apolhex[i] = float(
            atom_listhex[i][6]
        )  # atomic apolar contributions to solvation free enthalpy in hexadecane: SS G_CDS in kcal/mol

        # water:
        Polwat[i] = float(
            atom_listwat[i][3]
        )  # atomic polar contributions to solvation free enthalpy in water: Polarization Free Energy (G_P) in kcal/mol
        # in amsol-mod4 output:::
        # Apolwat[i] = float(atom_listwat[i][5]) # atomic apolar contributions to solvation free enthalpy in water: SS G_CDS in kcal/mol
        Apolwat[i] = float(
            atom_listwat[i][6]
        )  # atomic apolar contributions to solvation free enthalpy in water: SS G_CDS in kcal/mol

        ## sums over atomic contributions:

        Chghexsum += Chghex[
            i
        ]  # sum of the partial charges (CM2: Truhlar's charge model 2 charges) of each atom in hexadecane (!) solvent
        Polhexsum += Polhex[
            i
        ]  # sum of the atomic polar contributions to solvation free enthalpy: Polarization Free Energy (G_P)
        SAAsum += SAA[i]  # sum of the atomic surface area contibutions in Angstrom
        # is independent of the solvent under consideration
        # !!! former apolsum
        sum_Apolhex += Apolhex[
            i
        ]  # sum of the atomic apolar contributions to solvation free enthalpy: SS G_CDS from hexadecane calculation (!!!!)
        # !!! former energy_sum
        # in amsol-mod4 output:::
        ...
        FreeEnergyHex_sum += float(atom_listhex[i][7])
        # sum of the atomic contributions to
        # for hexadecane (!!!) solvent: In AMSOL7.1 output-table called: Subtotal (in kcal/mol)
    # logger.debug tothex
    # logger.debug "len(tothex) :::", len(tothex)
    # logger.debug "tothex :::", tothex
    # logger.debug "Chghexsum :::", Chghexsum
    # logger.debug "Polhexsum :::", Polhexsum
    # logger.debug "SAAsum :::", SAAsum
    # logger.debug "sum_Apolhex :::", sum_Apolhex
    # logger.debug "FreeEnergyHex_sum :::", FreeEnergyHex_sum
    #
    #
    ...
    cs_coeff = (float(tothex[4]) - sum_Apolhex) / float(
        tothex[3]
    )  # cs_coeff = total LS contribution / total AreaSAA
    ...
    # logger.debug ""
    # logger.debug "tothex[4] (SS G_CDS):::", float(tothex[4])
    # logger.debug "totwat[4] (SS G_CDS):::", float(totwat[4])
    # logger.debug "sum_Apolhex :::", sum_Apolhex
    # logger.debug "tothex[3] (Area) :::", float(tothex[3])
    # logger.debug "CS coeff ::: ", cs_coeff
    ...
    for i in range(N):  # do substraction
        diff_Pol[i] = (
            Polwat[i] - Polhex[i]
        )  # diff_Pol[i] is the difference of the atomic polar contribution to solvation free energy
        # obtained in water vs. the one obtained in hexadecane

        tot_diff_Pol = (
            tot_diff_Pol + diff_Pol[i]
        )  # tot_diff_Pol is the sum of all differences of atomic polar contributions to solvation
        # free energy obtained in water vs. in hexadecane

        sum_csTimesSAA = (
            sum_csTimesSAA + cs_coeff * SAA[i]
        )  # sum_i cs_coeff * SAA[i] = sum_i (LS contribution/ total Area) * atomic Surface Area SAA[i] = LS contribution

        # - (Apolhex[i] + cs_coeff * SAA[i]): minus since: wat MINUS hex. cs_coeff and LS contribution only present in hex (Large Solvent)
        diff_Apol[i] = Apolwat[i] - (
            Apolhex[i] + cs_coeff * SAA[i]
        )  # diff_Apol[i] is the difference of the atomic apolar contribution to solvation free energy
        # obtained in water vs. the one obtained in hexadecane minus cs_coeff * SAA[i]

        tot_diff_Apol = (
            tot_diff_Apol + diff_Apol[i]
        )  # tot_diff_Apol is sum of all atomic diff_Apol[i]

        diff_AtomicSolv[i] = (
            diff_Pol[i] + diff_Apol[i]
        )  # diff_AtomicSolv[i] is the sum of the atomic diff_Pol[i] and diff_Apol[i] (see above for diff_Pol[i] and diff_Apol[i])
        # diff_AtomicSolv[i] considers the difference between water and hexadecane results for each atom!!!

        tot_diff_PolPlusApol = (
            tot_diff_PolPlusApol + diff_AtomicSolv[i]
        )  # tot_diff_PolPlusApol is the sum over all atomic contributions to the difference of solvation free energies
    ...
    )  # formal charge is the same for water or hexadecane
    for i in range(N):
        file.write(
            "%8.4f%8.2f%7.2f%8.2f%8.2f\n"
            % (Chghex[i], diff_Pol[i], SAA[i], diff_Apol[i], diff_AtomicSolv[i])
        )
    file.close()

    logger.debug("**** The function diff_amsol71_files() was finished. ****")
...
def modify_charges_mol2_file(mol2file, atom_list_hex, outputprefix):
    ...
    logger.debug("**** starting the function modify_charges_mol2_file() *****")
    logger.debug(
        "CM2 charges from an AMSOL7.1 SM5.42R calculation in hexadecane (!!!) are written to mol2-file. Former charges are overwritten."
    )

    mol = read_Mol2_file(mol2file)[0]

    n = len(atom_list_hex)
    if n != len(mol.atom_list):
        logger.error(f"Error: n != len(mol.atom_list) : {n} != {len(mol.atom_list)}")
        sys.exit()

    for i in range(n):
        charge = atom_list_hex[i][2]
        logger.debug(f"{mol.atom_list[i].Q} {charge}")
        mol.atom_list[i].Q = float(charge)

    filename = outputprefix + ".mol2"
    write_mol2(mol, filename)
    logger.debug("**** The function modify_charges_mol2_file() was finished. ****")
    return


def process_amsol_mol2(filenamewat, filenamehex, mol2file, outputprefix):
    logger.debug("**** entering the main program in process_amsol71_mol2.py ****")
    atom_list_wat, tot_wat, name_wat, numat_wat = process_amsol_file(
        filenamewat, outputprefix, "wat"
    )
    atom_list_hex, tot_hex, name_hex, numat_hex = process_amsol_file(
        filenamehex, outputprefix, "hex"
    )
    ...
    if name_hex != name_wat or numat_hex != numat_wat:
        logger.debug("Error: Name or Atom counts do not agree")
        #
        # else:
        logger.debug("Wat-name      = " + name_wat)
        logger.debug("Wat-atom cout = " + str(numat_wat))
        logger.debug("Hex-name      = " + name_hex)
        logger.debug("Hex-atom cout = " + str(numat_hex))

    logger.debug("just before diff_amsol71_files() function")
    diff_amsol71_files(
        atom_list_wat,
        tot_wat,
        atom_list_hex,
    ...
    logger.debug("just before modify_charges_mol2_file() function")
    modify_charges_mol2_file(mol2file, atom_list_hex, outputprefix)

    logger.debug(
        "**** The main program in process_amsol71_mol2.py was finished for ****"
    )
    logger.debug("%s and %s." % (filenamewat, filenamehex))
    logger.debug("*******************************************************************")
    #
...
if __name__ == "__main__":
    if len(sys.argv) != 5:  # if no input
        logger.error(" This script needs the following:")
        logger.error(" (1) amsol water output file ")
        logger.error(" (2) amsol hex output file ")
        logger.error(" (3) amsol hex output file ")
        logger.error(" (4) outputprefix ")
        sys.exit()
    ...
    process_amsol_mol2(filenamewat, filenamehex, mol2file, outputprefix)
